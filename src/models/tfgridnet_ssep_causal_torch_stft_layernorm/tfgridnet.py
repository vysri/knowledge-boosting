...
class TFGridNet(AbsSeparator):
    ...
    def forward(
        ...
        for ii in range(self.n_layers):
            ...
            self.kd_activations.append(batch)
        ...
        self.kd_activations.append(batch.reshape(n_batch, self.n_srcs * 2, n_freqs, n_frames).transpose(2, 3))
        ...
        if self.knowledge_distillation:
            return_val.append(self.kd_activations)

        if self.generate_embeddings:
            return_val.append(embedding)
        
        return_val.append(input_state)
